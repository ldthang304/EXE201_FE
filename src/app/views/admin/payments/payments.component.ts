    ...
    <div class="space-y-6">
      ...
      <div class="bg-white rounded-lg shadow p-6">
        <div class="flex justify-between items-center">
          <div>
            <h1 class="text-3xl font-bold text-gray-900">Lịch sử giao dịch VNPay</h1>
            <p class="text-gray-600 mt-2">Xem và theo dõi tất cả giao dịch thanh toán qua VNPay</p>
          </div>
            ...
            <tbody class="bg-white divide-y divide-gray-200">
              <tr *ngFor="let payment of payments" class="hover:bg-gray-100">
                <td class="px-6 py-4 whitespace-nowrap">
                  ...
                  <div class="text-sm text-gray-500" *ngIf="payment.vnpayTransactionNo">
                    VNP: {{payment.vnpayTransactionNo}}
                  </div>
                </td>
                <td class="px-6 py-4 whitespace-nowrap">
                ...
                <td class="px-6 py-4 whitespace-nowrap">
                  <div class="text-sm text-gray-900">{{payment.order?.user?.username}}</div>
                ...
                <td class="px-6 py-4 whitespace-nowrap">
                  <div class="text-sm text-gray-900">{{payment.vnpayTransactionId || 'N/A'}}</div>
                </td>
                <td class="px-6 py-4 whitespace-nowrap">
                ...
                <td class="px-6 py-4 whitespace-nowrap text-sm font-medium space-x-2">
                  ...
                  <button 
                    *ngIf="payment.vnpayTransactionId"
                    (click)="checkVnpayStatus(payment.vnpayTransactionId)"
        ...
        <div class="relative top-20 mx-auto p-5 border w-11/12 md:w-3/4 lg:w-1/2 shadow-lg rounded-md bg-white" (click)="$event.stopPropagation()">
          <div class="mt-3">
            ...
            <div class="space-y-4">
              ...
              <!-- VNPay Info -->
              <div class="bg-blue-100 p-4 rounded-lg" *ngIf="selectedPayment.vnpayTransactionId || selectedPayment.vnpayTransactionNo">
                <h4 class="font-medium text-gray-900 mb-2">Thông tin VNPay</h4>
                <div class="grid grid-cols-2 gap-4">
                  <div>
                    <p *ngIf="selectedPayment.vnpayTransactionId"><strong>Transaction ID:</strong> {{selectedPayment.vnpayTransactionId}}</p>
                    <p *ngIf="selectedPayment.vnpayTransactionNo"><strong>Transaction No:</strong> {{selectedPayment.vnpayTransactionNo}}</p>
                  </div>
                  <div>
                    <p *ngIf="selectedPayment.paymentUrl"><strong>Payment URL:</strong> 
                      <a [href]="selectedPayment.paymentUrl" target="_blank" class="text-blue-600 hover:text-blue-800">Xem</a>
                    </p>
                  </div>
                </div>
              </div>
              ...
              <div class="bg-gray-100 p-4 rounded-lg" *ngIf="selectedPayment.order">
                ...
                <div class="grid grid-cols-2 gap-4">
                  <div>
                    ...
                    <p><strong>Khách hàng:</strong> {{selectedPayment.order.user?.username}}</p>
                  </div>
                  <div>
                    <p><strong>Email:</strong> {{selectedPayment.order.user?.email}}</p>
                  </div>
                </div>
              </div>
            </div>
...
export class AdminPaymentsComponent implements OnInit {
  ...
  searchPayments() {
    ...
    if (this.searchTerm) {
      const filtered = this.payments.filter(payment => 
        ...
        payment.vnpayTransactionId?.toLowerCase().includes(this.searchTerm.toLowerCase()) ||
        payment.vnpayTransactionNo?.toLowerCase().includes(this.searchTerm.toLowerCase())
      );
  ...
  checkVnpayStatus(transactionId: string) {
    // In production, this would call VNPay API to check transaction status
    alert(`Checking VNPay status for transaction: ${transactionId}`);
  }

  exportPayments() {
    // In production, this would export to Excel
    const csvContent = this.generateCSV();
    this.downloadCSV(csvContent, 'vnpay-transactions.csv');
  }

  generateCSV(): string {
    const headers = ['Mã giao dịch', 'Mã đơn hàng', 'Khách hàng', 'Số tiền', 'VNPay TxnID', 'Trạng thái', 'Ngày tạo'];
